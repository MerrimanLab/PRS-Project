...
tiff(file = here("Output/Plots/TophivsPRS.tiff"), units = "in", width = 11, height = 9.5, res = 300)
...
# Preparing to produce tiff file.
tiff(file = here("Output/Plots/TophivsPRS_adj.tiff"), units = "in", width = 11, height = 9.5, res = 300)
...
# Preparing to produce tiff file.
tiff(file = here("Output/Plots/TophivsPRS2.tiff"), units = "in", width = 11, height = 9.5, res = 300)
...
# Preparing to produce tiff file.
tiff(file = here("Output/Plots/TophivsPRS2_adj.tiff"), units = "in", width = 11, height = 9.5, res = 300)
...
# Preparing to produce tiff file.
tiff(file = here("Output/Plots/Tophivsrs2231142.tiff"), units = "in", width = 11, height = 9.5, res = 300)
...
# Preparing tophaceous gout vs rs2231142 (adjusted) model data for meta-analysis.
tmp <- TophiModels %>% 
  filter(Predictor == "rs2231142",
         str_detect(Covariates, "DURATION"),
         `N case` > 20,
...
# Preparing to produce tiff file.
tiff(file = here("Output/Plots/Tophivsrs2231142_adj.tiff"), units = "in", width = 11, height = 9.5, res = 300)
...
# Preparing to produce tiff file.
tiff(file = here("Output/Plots/Tophivsrs10011796.tiff"), units = "in", width = 11, height = 9.5, res = 300)
...
# Preparing tophaceous gout vs rs10011796 (adjusted) model data for meta-analysis.
tmp <- TophiModels %>% 
  filter(Predictor == "rs10011796",
         str_detect(Covariates, "DURATION"),
         `N case` > 20,
...
# Preparing to produce tiff file.
tiff(file = here("Output/Plots/Tophivsrs10011796_adj.tiff"), units = "in", width = 11, height = 9.5, res = 300)
...
table1 <- tibble("Cohort" = cohortstring[which(cohortstring %in% names(data_list2))], 
                 ...
                 "Urate PRS" = unlist(lapply(data_list2, function(x) report(x$PRS))))
...
# Extracting male model statistics.
tmp1 <- tmp %>% 
  filter(Sex == "Male") %>% 
  rename("Male OR" = OR,
         "Male CI" = CI,
...
# Extracting female model statistics.
tmp2 <- tmp %>% 
  filter(Sex == "Female") %>% 
  rename("Female OR" = OR,
         "Female CI" = CI,
...
# Combining male and female gout model results.
...
Comparing Gout PRS, Urate PRS, and unweighted PRS results in men only. Using each PRS, standardized in pooled cohorts of each ancestry, assessing their relative effects. Also producing standardized effect sizes using the lm.beta package and partial R-squared values.

```{r UratevGoutPRS_Male, eval = F}
# Extracting information on gout variants that aren't in urate PRS. Three don't match either loci or RSID.
different <- UKBB_Gene_OR %>% 
  filter(!(RSID %in% Tin_Gene_OR$RSID),
         !Locus_Name %in% Tin_Gene_OR$Locus_Name) %>% 
  select(CHR:BP, BP1:Locus_Name)

# Extracting variants from Tin PRS that are near the different hits, all three are accounted for.
tin_different <- Tin_Gene_OR %>% 
  filter(CHR %in% different$CHR, between(BP, min(different$BP1), max(different$BP2)))

# Cleaning up.
rm(different, tin_different)

# Making list of PRS names.
prsnames <- c("PRS_std", "Urate_PRS_std", "PRS2_std", "Urate_PRS2_std", "Unweighted_PRS_std")
...
for(i in seq_along(prsnames)){
  ...
  f <- as.formula(paste("AGE1ATK", prsnames[i], sep = " ~ "))
  f2 <- as.formula(paste("AGE1ATK", str_remove(prsnames[i], "_std"), sep = " ~ "))
  ...
  out1 <- list("Age at onset (years)",
               ...
               round(rsq.partial(get(paste0("mod", i)), adj = T)$partial.rsq, 3), 
...
# Modeling each standardized PRS against serum urate.
for(i in seq_along(prsnames)){
  
  # Making formula object.
  f <- as.formula(paste("URATE", prsnames[i], sep = " ~ "))
  f2 <- as.formula(paste("URATE", str_remove(prsnames[i], "_std"), sep = " ~ "))
...
# Modeling each standardized PRS against tophaceous gout.
for(i in seq_along(prsnames)){
  
  # Making formula object.
  f <- as.formula(paste("TOPHIGOUT", prsnames[i], sep = " ~ "))
  f2 <- as.formula(paste("TOPHIGOUT", str_remove(prsnames[i], "_std"), sep = " ~ "))
  
  # Running model.
  assign(paste0("mod", i), glm(f, family = binomial, data = tmp))
  
  # Extracting model summaries.
  out1 <- list("Tophaceous disease",
               prsnames[i],
               NA,
               round(exp(summary(get(paste0("mod", i)))$coefficients[[2, 1]]), 3), 
               round(exp(confint.default(get(paste0("mod", i)))[2,][[1]]), 3), 
               round(exp(confint.default(get(paste0("mod", i)))[2,][[2]]), 3),
               formatC(summary(get(paste0("mod", i)))$coefficients[[2, 4]], format = "e", digits = 2),
               NA, 
               NA,
               "European",
...
# Extracting only aus/nz gout cohort.
tmp2 <- tmp %>% 
  filter(COHORT_GOUT == "Aus/NZ - Gout")
...
for(i in seq_along(prsnames)){
  ...
  # Running model.
  assign(paste0("mod", i), glm(f, family = binomial, data = tmp2))
...
# Extracting only GlobalGout cohort.
tmp2 <- tmp %>% 
  filter(COHORT_GOUT == "GlobalGout - Gout")
...
# Extracting only Ardea LASSO cohort.
tmp2 <- tmp %>% 
  filter(COHORT_GOUT == "Ardea - LASSO")
...
# Extracting only Ardea CLEAR 1 cohort.
tmp2 <- tmp %>% 
  filter(COHORT_GOUT == "Ardea - CLEAR1")
...
# Extracting only Ardea CLEAR 2 cohort.
tmp2 <- tmp %>% 
  filter(COHORT_GOUT == "Ardea - CLEAR2")
...
# Extracting only Ardea LIGHT cohort.
tmp2 <- tmp %>% 
  filter(COHORT_GOUT == "Ardea - LIGHT")
...
final <- out %>% 
  ...
  select(Outcome:OR_std, `95%-CI`, P:Cohort)

# Writing out results.
write_delim(final, file = here("Output/Tables/weightingscheme.txt"), delim = "\t")
```

For age at onset, this suggests that the gout PRS is consistently the best for associations, though it is only marginally better than the urate PRS. The urate PRS is better when it only includes the 15 gout associated variants compared to the 82 from Tin, suggesting these may be adding noise to the model, and making the PRS less generalizable. The unweighted PRS is consistently the worst for both age at onset.

For urate models, the urate PRS is outperformed by the gout PRS for urate associations in Europeans, though this is not the case for East Polynesians, and West Polynesians cannot be interpreted as they were non-significant.

When considering tophaceous disease, it is not really appropriate to interpret pooled European models, but we can interpret the two Polynesian cohorts, and if we just look at Europeans in each cohort individually, we can say whether the PRS performance order is consistent. Based on the European models, it is not clear which PRS is better, though it does appear that all models show a weak positive effect on tophaceous gout. The Polynesian tophaceous disease models suggested that the 19 variant gout PRS performed best, followed by the 15 variant gout PRS, then unweighted, then the 15 variant urate PRS, then the 82 variant urate PRS.

I also ran the models in women only:

```{r UratevGoutPRS_Female, eval = F}
# Making list of PRS names.
prsnames <- c("PRS_std", "Urate_PRS_std", "PRS2_std", "Urate_PRS2_std", "Unweighted_PRS_std")
...
# Writing out results.
write_delim(final, file = here("Output/Tables/weightingscheme_female.txt"), delim = "\t")
```
